...
//!
//! Watches Maildir directories for changes and propagates them to the Index.
...
use crate::mailstore::watcher::maildir::{parse_maildir_filename, parse_maildir_path};
...
/// Filesystem-based watcher for Maildir directories
...
impl FilesystemWatcher {
    ...
    pub async fn new<P: AsRef<Path>>(root_path: P, index: Arc<dyn Index>) -> Result<Self> {
        ...
        task::spawn(watcher_loop(command_rx, root_clone, index_clone));
...
async fn watcher_loop(
    ...
    let (event_tx, event_rx) = bounded::<Result<Event>>(1000);

    // Spawn the filesystem event processor
    let state_clone = state.clone();
    let index_clone = index.clone();
    let root_clone = root.clone();
    task::spawn(event_processor(event_rx, state_clone, index_clone, root_clone));
    ...
    while let Ok(cmd) = command_rx.recv().await {
        match cmd {
            WatchCommand::Watch { username, mailbox, reply } => {
            ...
            WatchCommand::Unwatch { username, mailbox, reply } => {
                let result = handle_unwatch(
                    &mut watcher,
                    &state,
                    &username,
                    &mailbox,
...
async fn handle_watch(
    ...
    // Create mailbox directories if they don't exist (Maildir structure)
    ...
    scan_existing_messages(root, username, mailbox, index, &mut state).await?;
...
async fn handle_unwatch(
    ...
    if let Some(path) = state.watched_mailboxes.remove(&(username.to_string(), mailbox.to_string())) {
        {
            let mut watcher = watcher.lock().unwrap();
            watcher
                .unwatch(&path)
        ...
        Err(Error::NotFound(format!("Mailbox not being watched: {}/{}", username, mailbox)))
    }
}
...
async fn scan_existing_messages(
    ...
    for subdir in &["new", "cur"] {
        ...
        while let Some(entry) = entries.next().await {
            ...
            // Parse the message
            if let Some(maildir_msg) = parse_maildir_filename(std_path) {
                // Check if this message is already in the index
                let message_path = format!("{}/{}/{}", username, mailbox, maildir_msg.unique_id);
                ...
                // Parse email headers
                if let Ok(parsed) = mailparse::parse_mail(&content) {
                    ...
                    let body_preview = parsed.get_body().unwrap_or_default()
                        .chars()
                        .take(200)
                        .collect();

                    let mut flags = maildir_msg.flags.clone();
                    // Messages in 'new' directory should have the Recent flag
                    if maildir_msg.is_new {
                    ...
                    // Track the message
                    state.message_mapping.insert(maildir_msg.unique_id.clone(), message_id);
...
async fn event_processor(
    ...
    while let Ok(event_result) = event_rx.recv().await {
        match event_result {
            Ok(event) => {
                if let Err(e) = handle_filesystem_event(event, &state, &index, &root).await {
    ...
    match event.kind {
        EventKind::Create(_) => {
            ...
            for path in event.paths {
                handle_create_event(&path, state, index, root).await?;
            }
        }
        EventKind::Modify(_) => {
            // File modified (renamed in Maildir when flags change)
            for path in event.paths {
                handle_modify_event(&path, state, index, root).await?;
            }
        }
        EventKind::Remove(_) => {
            // File deleted
            for path in event.paths {
                handle_remove_event(&path, state, index, root).await?;
            }
        }
        _ => {
...
async fn handle_create_event(
    ...
    // Parse the path to extract username, mailbox, and message info
    if let Some((username, mailbox, _subdir)) = parse_maildir_path(path, root) {
        if let Some(maildir_msg) = parse_maildir_filename(path) {
            ...
            if let Ok(parsed) = mailparse::parse_mail(&content) {
                ...
                info!("Indexed new message: {}/{}/{}", username, mailbox, maildir_msg.unique_id);
            }
        }
    }
...
/// Handle file modification event (typically a rename in Maildir for flag changes)
async fn handle_modify_event(
    ...
    // In Maildir, flag changes result in file renames
    // We need to find the message by its unique_id and update its flags

    if let Some((username, mailbox, _subdir)) = parse_maildir_path(path, root) {
        if let Some(maildir_msg) = parse_maildir_filename(path) {
            let state_read = state.read().await;
            if let Some(&message_id) = state_read.message_mapping.get(&maildir_msg.unique_id) {
                drop(state_read);
                ...
                index.update_flags(message_id, flags).await?;
                info!("Updated message flags: {}/{}/{}", username, mailbox, maildir_msg.unique_id);
            }
        }
    }
...
async fn handle_remove_event(
    ...
    if let Some((username, mailbox, _subdir)) = parse_maildir_path(path, root) {
        if let Some(maildir_msg) = parse_maildir_filename(path) {
            ...
            if let Some(message_id) = state_write.message_mapping.remove(&maildir_msg.unique_id) {
                drop(state_write);
                ...
                info!("Removed message from index: {}/{}/{}", username, mailbox, maildir_msg.unique_id);
            }
        }
    }
...
mod tests {
    ...
    async fn test_watcher_creation() {
        ...
        let watcher = FilesystemWatcher::new(tmp_dir.path(), index).await;
    ...
    async fn test_watch_mailbox() {
        ...
        let watcher = FilesystemWatcher::new(tmp_dir.path(), index).await.unwrap();
